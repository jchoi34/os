...
sys_execv(char *progname, char **args, int *err)
{
	...
	vaddr_t stack_offset, str_addr;
	pad = ((stackptr - total_len) - ((argc + 1) * sizeCharPointer)) % 8;
	str_addr = stackptr;
	stack_offset = (stackptr - total_len) - pad;
	stack_offset -= sizeCharPointer;
//	argv[i] = NULL;
	userptr_t argv = NULL;
	result = copyout((const void *) &argv, (userptr_t) stack_offset, sizeCharPointer);
	if(result) {
		*err = result;
		V(sem_exec);
		kfree(args_buffer);
	...
	offset = 0;
	stack_offset -= argc * sizeCharPointer;
	stackptr = stack_offset;
	total_len = 0;
	// copy out argv (pointers to the string arguments)
	for(i = 0; i <argc; i++) {
		kprintf("\ni: %d s: %s\n", i, args_buffer+size+1);
		size = strlen(args_buffer + offset) + 1;
		total_len += size;
		kprintf("\ns: %s\n", (char *)str_addr-total_len);
		result = copyout((const void *) (&str_addr - total_len), (userptr_t)stack_offset, sizeCharPointer);
		stack_offset += sizeCharPointer;
		if(result) {
			*err = result;
			V(sem_exec);
